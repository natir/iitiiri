    ...
    f64: num_traits::AsPrimitive<P>,
{
    fn train(data: &[node::Node<P, O>]) -> Self {
        ...
        //let mut domain2level2begin_index = vec![vec![Vec::new(); AFFINE_TRAIN_LEVEL_LEN]; N];
...
mod tests {
    ...
    fn affine() {
        ...
        let estimator = AffineEstimator::<usize, 4>::train(&data);

        assert_eq!(
            <AffineEstimator<usize, 4> as Estimator<usize, bool>>::guess(&estimator, 500, 150),
            15
        )
    }
}
